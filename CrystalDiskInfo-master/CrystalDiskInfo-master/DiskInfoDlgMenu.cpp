...
void CDiskInfoDlg::OnSendToServer()
{
	...
	int serverCount = CDataCollector::GetServers(m_Ini, servers);
	
	ServerInfo selectedServer;
	
	if (serverCount == 0) {
		// Если серверов нет, используем старый формат или создаем дефолтный
		...
		selectedServer.name = _T("Default Server");
		selectedServer.host = serverHost;
		selectedServer.port = serverPort;
		selectedServer.isDefault = true;
	} else {
		// Если несколько серверов, выбираем сервер по умолчанию
		int defaultIndex = -1;
		...
		if (defaultIndex >= 0) {
			// Используем сервер по умолчанию
			selectedServer = servers[defaultIndex];
		} else {
	...
	CString confirmMsg;
	if (serverCount > 1) {
		// Если несколько серверов, показываем название
		CString defaultTag = selectedServer.isDefault ? _T(" [Default]") : _T("");
		confirmMsg.Format(_T("Send data about %d disks to server?\n\n%s%s\n%s:%d\n\n(If unavailable, will try other servers)"), 
		                 (int)m_Ata.vars.GetCount(), selectedServer.name, defaultTag, 
		                 selectedServer.host, selectedServer.port);
	} else {
		// Если один сервер, просто показываем URL
		CString serverUrl;
		serverUrl.Format(_T("http://%s:%d/api/hdd_collect"), selectedServer.host, selectedServer.port);
		confirmMsg.Format(_T("Send data about %d disks to server?\n\n%s"), 
	...
	// Отправка данных с автоматическим переключением на следующий сервер при ошибке
	CString response;
	CWaitCursor wait;
	
	// Формируем список серверов для попыток отправки
	std::vector<ServerInfo> serversToTry;
	if (serverCount > 0) {
		// Добавляем сервер по умолчанию первым
		for (size_t i = 0; i < servers.size(); i++) {
			if (servers[i].isDefault) {
				serversToTry.push_back(servers[i]);
				break;
			}
		}
		// Затем добавляем остальные серверы
		for (size_t i = 0; i < servers.size(); i++) {
			if (!servers[i].isDefault) {
				serversToTry.push_back(servers[i]);
			}
		...
		// Если серверов нет в новом формате, используем старый
		serversToTry.push_back(selectedServer);
	}
	
	// Пробуем отправить на каждый сервер по очереди
	bool success = false;
	CString lastError;
	ServerInfo successfulServer;
	CString comment = _T(""); // Комментарий пока пустой, можно добавить диалог позже
	
	for (size_t i = 0; i < serversToTry.size(); i++) {
		CString serverUrl;
		serverUrl.Format(_T("http://%s:%d/api/hdd_collect/v2"), serversToTry[i].host, serversToTry[i].port);
		
		if (m_DataCollector.SendToServer(serversToTry[i].host, serversToTry[i].port, m_Ata, response, comment)) {
			// Успешная отправка
			success = true;
			successfulServer = serversToTry[i];
			break;
		} else {
			// Сохраняем ошибку для последнего сервера
			lastError = response;
			// Продолжаем попытки со следующим сервером
		}
	}
	
	if (success) {
		// Показываем успешное сообщение с информацией о сервере
		CString successMsg;
		successMsg.Format(_T("Data successfully sent to server!\n\n%s\n%s:%d"), 
		                 successfulServer.name, successfulServer.host, successfulServer.port);
		AfxMessageBox(successMsg, MB_OK | MB_ICONINFORMATION);
	} else {
		// Все серверы недоступны
		CString errorMsg;
		if (serversToTry.size() > 1) {
			errorMsg.Format(_T("Failed to send data to all servers (%d servers tried).\n\nLast error:\n%s"), 
			               (int)serversToTry.size(), lastError);
		} else {
			errorMsg.Format(_T("Error sending data to server:\n\n%s\n%s:%d\n\nResponse: %s"), 
			               selectedServer.name, selectedServer.host, selectedServer.port, lastError);
		}
		AfxMessageBox(errorMsg, MB_OK | MB_ICONERROR);
	}
}

void CDiskInfoDlg::AutoSendToServerSilent() {
	// Тихая отправка данных на сервер без показа диалогов (для автоматической отправки при запуске)
	if (m_Ata.vars.GetCount() == 0) {
		return; // Нет дисков для отправки
	}
	
	// Читаем флаг автоматической отправки из конфигурации
	int autoSendOnStartup = GetPrivateProfileInt(_T("Server"), _T("AutoSendOnStartup"), 0, m_Ini);
	if (autoSendOnStartup == 0) {
	...
	if (serverCount == 0) {
		return; // Нет настроенных серверов
	}
	
	// Определяем сервер для отправки (сервер по умолчанию или первый)
	ServerInfo selectedServer;
	if (serverCount == 1) {
		selectedServer = servers[0];
	} else {
		// Ищем сервер по умолчанию
		int defaultIndex = -1;
		for (size_t i = 0; i < servers.size(); i++) {
			if (servers[i].isDefault) {
				defaultIndex = (int)i;
	...
	// Формируем список серверов для попыток отправки (с сервером по умолчанию первым)
	std::vector<ServerInfo> serversToTry;
	for (size_t i = 0; i < servers.size(); i++) {
		if (servers[i].isDefault) {
			serversToTry.push_back(servers[i]);
	...
	// Пробуем отправить на каждый сервер по очереди (без показа сообщений)
	CString response;
	CString comment = _T(""); // Комментарий пустой при автоматической отправке
	
	for (size_t i = 0; i < serversToTry.size(); i++) {
		if (m_DataCollector.SendToServer(serversToTry[i].host, serversToTry[i].port, m_Ata, response, comment)) {
			// Успешная отправка - выходим без показа сообщений
			break;
		}
		// При ошибке продолжаем попытки со следующим сервером без показа ошибок
	}
	
	// Все происходит в фоновом режиме, никаких диалогов не показываем
}
